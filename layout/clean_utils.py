...
def extract_categories(raw_item):
    """
    ...
    for widget in objects:
        ...
        widget_objects = widget.get("objects", [])
        ...
        for obj in widget_objects:
            ...
            for group in products:
                ...
                category_type = product.get("type")

                l0 = product.get("level0_category", [{}])[0].get("id")

                l1 = product.get("level1_category", [{}])[0].get("id")

                if not category_type or not l0 or not l1:
                    continue
                ...
                if key not in category_map:
                    ...
                    category_map[key] = {
                        "category": slugify(category_type),
                        "display_name": category_type,
                        "l0_cat": str(l0),
                        "l1_cat": str(l1),
                        "url": (
                            "https://blinkit.com/"
...
if __name__ == "__main__":
    ...
    print(f"Saved " f"{len(cleaned)} " f"feeds → " f"{OUTPUT_FILE}")
